<template>
  <div id="seach">
    <header class="header">
      <span class="goBack" @click="backLastPage">
        <i class="rsiconfont rsicon-qiehuanqizuo"></i>
      </span>
      <label class="seachIntOut">
        <i class="rsiconfont rsicon-sousuo"></i>
        <input type="text" class="seachInt" placeholder="搜索" v-model="query.keyWord" />
      </label>
    ...
    <main class="main" v-if="!seachInit">
      <mt-navbar v-model="defaultTab">
        <mt-tab-item id="product">
          <i class="rsiconfont rsicon-shangpin"></i>商品</mt-tab-item>
        <mt-tab-item id="shop">
          <i class="rsiconfont rsicon-dianpu"></i>店铺</mt-tab-item>
      </mt-navbar>
        ...
        <mt-tab-container-item id="product">
          <div class="seachInit" v-if="seachInit">
            ...
            <ul class="history-seach" v-show="historySeachList.length > 0">
              <li v-for="(item,index) in historySeachList" :key="index">
                <i class="rsiconfont rsicon-naozhong"></i>{{item.value}}</li>
            </ul>
            <div>
              <button @click="clearHistory" v-show="historySeachList.length > 0">清空历史记录</button>
            </div>
