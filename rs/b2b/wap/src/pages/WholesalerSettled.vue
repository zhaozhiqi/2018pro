...
export default {
  ...
  data() {
    return {
      ...
      regionHold: [{ code: 330000 }, { code: 330100 }, { code: 0 }],
      regionToolbar: {
        name: '确认'
      },
      regionVisible: false,
  ...
  methods: {
    ...
    addressChange(picker, values) {
      ...
      //console.log(values,'.....')
      this.store.salesAreaCode._value = values[0]["label"] + values[1]["label"] + values[2]["label"]
      if (this.regionAble === true) {
        //取值并赋值
        this.store.salesAreaCode.value = values[0]["label"] + values[1]["label"] + values[2]["label"]
      }
      ...
      this.store.salesAreaCode.code = this.myAddressSlots.districtCode
    },
